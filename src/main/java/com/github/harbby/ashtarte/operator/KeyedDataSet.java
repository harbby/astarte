...
public class KeyedDataSet<KEY, ROW>
    ...
    public DataSet<Tuple2<KEY, Double>> avg(Mapper<ROW, Double> aggIf) {
        ...
        Operator<Tuple2<KEY, Double>> kvDs = dataSet.kvDataSet(x ->
        ...
        return new FullAggOperator<>(shuffleReducer, reducer);
    }

    @Override
    public <VALUE> DataSet<Tuple2<KEY, VALUE>> map(Mapper<Iterable<ROW>, VALUE> mapperReduce) {
        Partitioner<KEY> partitioner = new HashPartitioner<>(dataSet.numPartitions());
        Operator<Tuple2<KEY, ROW>> kvDs = dataSet.kvDataSet(x -> new Tuple2<>(keyGetter.map(x), x));
    ...
    public <VALUE> DataSet<Tuple2<KEY, VALUE>> agg(Mapper<ROW, VALUE> aggIf, Reducer<VALUE> reducer) {
        Operator<Tuple2<KEY, VALUE>> kvDs = dataSet.kvDataSet(x ->
                new Tuple2<>(keyGetter.map(x), aggIf.map(x)));
